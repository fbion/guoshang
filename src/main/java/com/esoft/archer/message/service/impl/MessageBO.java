...
public class MessageBO {
	...
	public void sendMsg(User user, String userMessageNodeId,
		...
		List<UserMessageTemplate> umts = getUserMessageTemplates(user, userMessageNodeId);
	...
	public void sendMsgWithNoException(User user, String userMessageNodeId,
			Map<String, String> params) {
		List<UserMessageTemplate> umts = getUserMessageTemplates(user, userMessageNodeId);
		for (UserMessageTemplate umt : umts) {
			String msg = replaceParams(umt, params);
			try{
				if (umt.getUserMessageWay().getId().equals(MessageConstants.UserMessageWayId.EMAIL) && StringUtils.isNotEmpty(user.getEmail())) {
					// 邮件
					mailService.sendMail(user.getEmail(), umt.getName(), msg);
				} else if (umt.getUserMessageWay().getId().equals(MessageConstants.UserMessageWayId.SMS) && StringUtils.isNotEmpty(user.getMobileNumber())) {
					// 短信
					smsService.send(msg, user.getMobileNumber());
				} else if (umt.getUserMessageWay().getId()
						.equals(MessageConstants.UserMessageWayId.LETTER)) {
	...
	private List<UserMessageTemplate> getUserMessageTemplates(User user, String userMessageNodeId){
		// 找到node与userid所有的template，外加必须发送的template，然后一一发送。
				String hql = "select distinct umt from UserMessage um right join um.userMessageTemplate umt where "
				...
				return ht.findByNamedParam(hql, new String[] {
