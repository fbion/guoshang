...
public class UserInfoHome extends EntityHome<User> implements Serializable {
	...
	public void findPwdByEmail(String jsCode){
		User l = userBO.getUserByEmail(findPwdEmail);
		if(l == null ){
			FacesUtil.getCurrentInstance().validationFailed();
			FacesUtil.addErrorMessage("该邮箱未注册");
			return;
		}
		this.setInstance(l);
		...
		params.put("authCode", authService.createAuthInfo(l.getId(), findPwdEmail, null, CommonConstants.AuthInfoType.FIND_LOGIN_PASSWORD_BY_EMAIL).getAuthCode());
		// 发送手机验证码
		messageBO.sendEmail(getBaseService().get(UserMessageTemplate.class, MessageConstants.UserMessageNodeId.FIND_LOGIN_PASSWORD_BY_EMAIL + "_email"), params, findPwdEmail);
		FacesUtil.addInfoMessage("验证码已经发送至邮箱！");
		RequestContext.getCurrentInstance().execute(jsCode);
	}
	
	public void findPwdByEmail1(){
		try {
			authService.verifyAuthInfo(getInstance().getId(), findPwdEmail,
					authCode,
					CommonConstants.AuthInfoType.FIND_LOGIN_PASSWORD_BY_EMAIL);
			this.step = 2;
		} catch (NoMatchingObjectsException e) {
