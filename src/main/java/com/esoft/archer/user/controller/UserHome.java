...
import com.esoft.core.jsf.message.OpenUrlFacesMessage;
import com.esoft.core.jsf.util.FacesUtil;
import com.esoft.core.util.CheckUtil;
...
public class UserHome extends EntityHome<User> implements java.io.Serializable {
    ...
    @Resource
    VoiceCodeService voiceCodeService;
     ...
     * 验证认证码是否正确.
     */
    private boolean correctAuthCode;
    @Logger
    static Log log;
     ...
     * 注册完成后是否默认登录.
     */
    private boolean isLoginAfterRegister;
    @Resource
    ...
    public String modifyPassword() {
        ...
        try {
            ...
            userService.modifyPassword(loginUser.getLoginUserId(),
                    getInstance().getPassword());
            FacesUtil.addMessage(null, new OpenUrlFacesMessage(
                    FacesMessage.SEVERITY_INFO, "密码修改成功，请重新登录！", null,
                    FacesUtil.getHttpServletRequest().getContextPath()
                            + "/user/center", false));
            return "pretty:userCenter";
        } catch (UserNotFoundException e) {
            FacesUtil.addErrorMessage("用户未登录！");
        ...
        try {
            if (getInstance().getReferrer() != null) {
                ...
                try {
                    ...
                    //如果用户名未找到用户,就通过手机号查找
                    try {
                        User user = userService.getUserByMobileNumber(referrer);
                        getInstance().setReferrer(user.getUsername());
                    } catch (UserNotFoundException e1) {
                        //如果手机号也没找到,就记录用户填写的推荐人
                    }
                }
            }
    ...
    public String saveBorrower() {
        ...
        userService.createBorrowerByAdmin(getInstance());
     ...
     * @param jsCode       成功后执行的js代码
     */
    public void sendRegisterAuthVoiceCode(String mobileNumber, String jsCode) {
        String code = authService.createAuthInfo(null, mobileNumber, null,
                CommonConstants.AuthInfoType.REGISTER_BY_MOBILE_NUMBER)
                .getAuthCode();
        voiceCodeService.send(code, mobileNumber);
        FacesUtil.addInfoMessage("请接听电话，查收验证码！");
        RequestContext.getCurrentInstance().execute(jsCode);
    }
    ...
    private void refreshRandomCode() {
        randomCode = RandomStringUtils.random(8, true, true);
    }
    ...
    public String getvCodeClient() {
        return vCodeClient;
    }

    public void setvCodeClient(String vCodeClient) {
