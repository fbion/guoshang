	...
	public void refuseWithdrawCashApply(WithdrawCash withdrawCash) {
		...
		if (wdc.getStatus().equals(WithdrawStatus.RECHECK)
			...
			try {
				...
				userBillBO.unfreezeMoney(wdc.getUser().getId(), wdc.getMoney(), OperatorInfo.REFUSE_APPLY_WITHDRAW, "提现申请被拒绝，解冻提现金额, 提现ID:" + withdrawCash.getId());
				userBillBO.unfreezeMoney(wdc.getUser().getId(), wdc.getFee(), OperatorInfo.REFUSE_APPLY_WITHDRAW, "提现申请被拒绝，解冻手续费, 提现ID:" + withdrawCash.getId());

			} catch (InsufficientBalance e) {
				throw new RuntimeException(e);
			}
			if (log.isInfoEnabled())
				log.info("提现审核复核未通过，提现编号：" + wdc.getId());
		}
	}
