...
@Service
public class YeePayWithdrawNofreezeOperation extends
	...
	public TrusteeshipOperation createOperation(WithdrawCash withdrawCash,
		...
		withdrawCash.setId(withdrawCashService.generateId());
...
//				withdrawCashService.refuseWithdrawCashApply(wc);
	...
	public void success(WithdrawCash wc, TrusteeshipOperation to) throws InsufficientBalance{
		wc.setStatus(UserConstants.WithdrawStatus.SUCCESS);
		ub.transferOutFromBalance(wc.getUser().getId(),
				wc.getMoney(), OperatorInfo.WITHDRAW_SUCCESS,
				"提现申请通过，取出冻结金额, 提现ID:" + wc.getId());
		ub.transferOutFromBalance(wc.getUser().getId(),
				wc.getFee(), OperatorInfo.WITHDRAW_SUCCESS,
				"提现申请通过，取出冻结手续费, 提现ID:" + wc.getId());
		sbs.transferInto(wc.getFee(), OperatorInfo.WITHDRAW_SUCCESS,
