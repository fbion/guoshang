...
public class TransferServiceImpl implements TransferService {
	...
	public void applyInvestTransfer(TransferApply ta)
		...
		// (负的本债权未收利息-未还投资手续费)*转让本金比例 <= 折让金 <= (本债权未收本金-转让手续费)*转让本金比例
		double minPremium = ArithUtil.mul(ta.getCorpus()/ta.getInvest().getMoney(), ArithUtil.add(-investRRM.getUnPaidInterest(),
				investRRM.getUnPaidFee()));
		double maxPremium = ArithUtil.mul(ta.getCorpus()/ta.getInvest().getMoney(), ArithUtil.sub(investRRM.getUnPaidCorpus(),
				calculateFee(ta)));
		if (minPremium > ta.getPremium()
				|| ta.getPremium() > maxPremium) {
...
//			    还款本金*转让本金比例*购买本金比例
	...
	public boolean canTransfer(String investId) throws InvestTransferException {
		...
		//所有用体验金的投资，都不允许债权转让
		if (invest.getUserCoupon() != null && invest.getUserCoupon().getCoupon().getType().equals(CouponConstants.Type.EXPERIENCE)) {
			return false;
		}
	...
	@Transactional(rollbackFor = Exception.class)
	public void delay(String transferApplyId,Date newExpectTime) throws InvalidExpectTimeException{
		
		TransferApply ta = ht.get(TransferApply.class, transferApplyId);
		ht.evict(ta);
		ta = ht.get(TransferApply.class, transferApplyId, LockMode.UPGRADE);
		if (!newExpectTime.after(new Date())) {
			throw new  InvalidExpectTimeException("预计执行时间必须在当前时间之后");
		} 
		
		// 添加债权到期调度任务
		ta.setDeadline(newExpectTime); 
		ta.setStatus(TransferStatus.TRANSFERING);
		ht.merge(ta);
		try {
	...
	public boolean isOverExpectTime(String transferApplyId) {
		TransferApply ta = ht.get(TransferApply.class, transferApplyId);
		if (new Date().before(ta.getDeadline())) {
			return false;
		}
		return true;
	}
