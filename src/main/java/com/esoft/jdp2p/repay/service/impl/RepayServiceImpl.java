...
public class RepayServiceImpl implements RepayService {
	...
	public void normalRepay(String repayId) throws InsufficientBalance,
		...
		normalRepay(repay, repay.getLoan().getUser().getId());
	}		
	...
	private void gRepays(Loan loan, NormalRepayCalculator normalRepayCalculator) {
		...
		for (Invest im : invests) {
			List<Repay> repays = normalRepayCalculator.generateRepays(
					...
					loan.getRateIBOI(), loan.getDeadline(),
					loanType.getRepayTimeUnit(), loanType.getRepayTimePeriod(),
					loan.getInterestBeginTime(), loanType.getInterestType(),
					loanType.getInterestPoint(), null);
			// 保存投资的还款信息
			...
			for (Repay repay : repays) {
				...
				if (im.getCouponMoney() != null&& im.getUserCoupon()!=null
						&& im.getUserCoupon().getCoupon().getType()
								.equals(CouponConstants.Type.EXPERIENCE)
						&& im.getCouponMoney() != 0) {
					// 计算因为使用了体验金，需要给系统多少本金
					double corpusToSystem = ArithUtil.round(im.getCouponMoney()
							/ im.getInvestMoney() * repay.getCorpus(), 2);
					investRepay.setCorpusToSystem(corpusToSystem);
					investRepay.setCorpus(ArithUtil.sub(repay.getCorpus(),
							corpusToSystem));
				} else {
	...
	public void advanceRepay(String loanId) throws InsufficientBalance,
		...
		ht.evict(loan);
		loan = ht.get(Loan.class, loanId, LockMode.UPGRADE);
		if (loan.getStatus().equals(LoanStatus.REPAYING)
				|| loan.getStatus().equals(LoanStatus.WAIT_REPAY_VERIFY)) {
			// 查询当期还款是否已还清
			...
			systemBillService.transferInto(ArithUtil.add(feeToSystem, feeAll),
					...
					"提前还款罚金及借款手续费到账，项目ID:" + loan.getId());
	...
	public void autoRepay() {
		...
		for (LoanRepay lr : lrs) {
			...
			if (repayDay.before(new Date())) {
				// 到还款日了，自动扣款
				if (autoRepayLoaner.equals("1")&&lr.getStatus().equals(LoanConstants.RepayStatus.REPAYING.toString())) {
					log.info("start autoRepay, loanRepayId: " + lr.getId());
					// 开启自动还款(借款者账户)
					try {
						normalRepay(lr.getId());
						continue;
				...
				if (autoRepayGuarantee.equals("1")) {
					log.info("start compensation, loanRepayId: " + lr.getId());
					// 开启代偿
					if (lr.getLoan().getCompensationUser() != null) {
						...
						try {
							...
							if (lr.getStatus().equals(LoanConstants.RepayStatus.OVERDUE.toString())) {
								compensationService.overdueRepay(lr.getId(), lr.getLoan().getCompensationUser().getId());
							}
							continue;
						} catch (NormalRepayException e) {
							throw new RuntimeException(e);
	...
	public void repayAlert() {
		...
		if (log.isInfoEnabled()) {			
			...
			log.info("repay alert start, Date:"+ simpleDateFormat.format(new Date()));
			log.info("repay alert start, daysBefore:"+daysBefore);
		}
		// 还款提醒。。。
		for (LoanRepay lr : lrs) {
