...
public class NormalRepayRFCLCalculator implements NormalRepayCalculator {
	...
	public List<Repay> generateRepays(double investMoney, Date investTime,
			double rate, Double rateIBOI, Integer deadline,
			String repayTimeUnit, Integer repayTimePeriod,
			Date interestBeginTime, String interestType,
	...
	private List<Repay> generateDayMonthRepays(double investMoney,
			Date investTime, double rate, Double rateIBOI, Integer deadline,
		...
		for (int i = 1; i <= deadline; i++) {
			...
			if (i == 1
				...
				intervalDays = DateUtil.calculateIntervalDays(
						interestBeginTime, investTime) + 1;
				// investBeginTime当前计息，还款日当天不计息
				if (intervalDays > 0) {
					// 如果计息日在投资日之前，则intervalDays为正数
					ir.setInterest(ArithUtil.round(
							rate * (interestDays - intervalDays) * investMoney
									/ 365, 2));
				} else {
					// 如果投资在计息日之前，则intervalDays为负数，应该加上按照即投即生息利率进行计算的intervalDays
					ir.setInterest(ArithUtil.round(
							(rate * interestDays - rateIBOI * intervalDays)
									* investMoney / 365, 2));
				}
			} else {
	...
	private List<Repay> generateDayDayRepays(double investMoney,
		...
		for (int i = 1; i <= deadline; i++) {
			...
			if (i == 1
				...
				if (intervalDays > 0) {
					// 如果计息日在投资日之前，则intervalDays为正数
					ir.setInterest(ArithUtil.round(rate
							* (repayTimePeriod - intervalDays) * investMoney
							/ 365, 2));
				} else {
					// 如果投资在计息日之前，则intervalDays为负数，应该加上按照即投即生息利率进行计算的intervalDays
					ir.setInterest(ArithUtil.round(
							(rate * repayTimePeriod - rateIBOI * intervalDays)
									* investMoney / 365, 2));
				}
			} else {
				// 金额*还款周期*还款周期单位*还款周期单位利率 = 利息
				ir.setInterest(ArithUtil.round(rate * repayTimePeriod
						* investMoney / 365, 2));
			}
			ir.setLength(repayTimePeriod);
			ir.setPeriod(i);
