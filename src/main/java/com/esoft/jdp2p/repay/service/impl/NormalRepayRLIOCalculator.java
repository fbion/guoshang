...
public class NormalRepayRLIOCalculator implements NormalRepayCalculator {
		...
		if (interestBeginPoint.equals(InterestPoint.INTEREST_BEGIN_ON_INVEST)) {
			...
			if (intervalDays > 0) {
				...
				ir.setInterest(ArithUtil.round(rate
						* (interestDays - intervalDays) * investMoney / 365, 2));
			} else {
				// 如果投资在计息日之前，则intervalDays为负数，应该加上按照即投即生息利率进行计算的intervalDays
				ir.setInterest(ArithUtil.round((rate * interestDays - rateIBOI
						* intervalDays)
						* investMoney / 365, 2));
			}
		} else {
			// 金额*还款周期*还款周期单位*还款周期单位利率 = 利息
			ir.setInterest(ArithUtil.round(rate * interestDays * investMoney
					/ 365, 2));
		}
	...
	private List<Repay> generateDayDayRepays(double investMoney,
		...
		if (interestBeginPoint.equals(InterestPoint.INTEREST_BEGIN_ON_INVEST)) {
			...
			if (intervalDays > 0) {
				// 如果计息日在投资日之前，则intervalDays为正数
				ir.setInterest(ArithUtil.round(rate * (deadline - intervalDays)
						* investMoney / 365, 2));
			} else {
				// 如果投资在计息日之前，则intervalDays为负数，应该加上按照即投即生息利率进行计算的intervalDays
				ir.setInterest(ArithUtil.round((rate * deadline - rateIBOI
						* intervalDays)
						* investMoney / 365, 2));
			}
		} else {
