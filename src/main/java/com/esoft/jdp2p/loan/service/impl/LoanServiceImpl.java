...
import com.esoft.archer.risk.SystemBillConstants.SystemOperatorInfo;
import com.esoft.archer.risk.SystemBillConstants.Type;
...
public class LoanServiceImpl implements LoanService {
	...
	@Resource
	SystemBillService systemBillService; // XXX: guoyw 20150314
		...
		try {
			for (Invest investment : invests) {
				...
				if (investment.getStatus().equals(
					...
					// 红包变为可用
					if (investment.getUserCoupon() != null) {
						userCouponService.enable(investment.getUserCoupon()
								.getId());
					}
					// 解冻金额为investMoeny-红包金额
					double realMoney = ArithUtil.sub(investment.getMoney(),
							investment.getCouponMoney() == null ? 0d : investment.getCouponMoney());
					ubs.unfreezeMoney(investment.getUser().getId(), realMoney,
	...
	public void giveMoneyToBorrower(String loanId)
		...
		for (Invest invest : invests) {
			...
			if (invest.getStatus().equals(
				...
				// XXX: guoyw,20150326,生日标处理代码
				// if ("sr".equals(loan.getLoanActivityType())) {
				// 用户投资日期 生日日期
				int ivtMonth = DateUtil.getMonth(invest.getTime());
				int ivtUserMonth = DateUtil.getMonth(invest.getUser()
						.getBirthday());
				int ivtDay = DateUtil.getDay(invest.getTime());
				int ivtUserDay = DateUtil
						.getDay(invest.getUser().getBirthday());
				if (ivtMonth == ivtUserMonth && ivtDay == ivtUserDay) {
					/**
					...
					try {
						brStr = configService
								.getConfigValue("birthday_rate");
					} catch (Exception e) {
						
					}
					double birthdayRateMoney = ArithUtil.mul(invest.getMoney(),
							Double.parseDouble(brStr));
					systemBillService.transferOut(birthdayRateMoney,
							SystemOperatorInfo.ADMIN_OPERATION,
							Type.OUT_FOR_USER_BIRTHDSRRATEMONEY + ",借款ID："
									+ loan.getId());
					ubs.transferIntoBalance(invest.getUser().getId(),
							birthdayRateMoney, OperatorInfo.ADMIN_OPERATION,
							"投资成功，发放生日奖励金额, 借款ID：" + loan.getId());
				}
				// }
				...
				try {
					...
					if (invest.getUserCoupon() != null) {
						...
						// 从系统账户转出红包使用的金额
						sbs.transferOut(Math.min(invest.getInvestMoney(),
								invest.getUserCoupon().getCoupon().getMoney()),
								OperatorInfo.USE_COUPON, "红包："
										+ invest.getUserCoupon().getCoupon()
												.getName());
					} else {
						ubs.transferOutFromFrozen(invest.getUser().getId(),
								invest.getMoney(),
	...
	@Override
	@Transactional(readOnly = false, rollbackFor = Exception.class)
	public void passApply(Loan loan) throws InvalidPublishTimeException,
			InvalidExpectTimeException, InsufficientBalance {
		// FIXME:验证
		...
		// XXX:guoyw,20150314
		if (null != loan.getPublishTime()) {
			// 预计发布时间 在 当前时间之前，抛异常
			if (!loan.getPublishTime().after(new Date())) {
				throw new InvalidPublishTimeException();
			}
			// 预计发布时间 在 预计执行时间之前，抛异常
			if (!loan.getPublishTime().before(loan.getExpectTime())) {
				throw new InvalidExpectTimeException();
			}
		}
		setPics(loan);
		...
		if (null != loan.getPublishTime()) {
			// 审核通过 项目预发布
			loan.setStatus(LoanConstants.LoanStatus.WAITING_RAISE);
			loan.setVerifyTime(new Date());
			ht.merge(loan);
			addAutoPublishLoanJob(loan);
			if (log.isDebugEnabled())
				log.debug(MessageFormat.format(
						"借款[编号：{0},名称：{1}]预发布审核通过!审核人：{2}", loan.getId(),
						loan.getName(), "//FIXME:"));
		} else {
			// 审核通过
			loan.setStatus(LoanConstants.LoanStatus.RAISING);
			...
			if (loan.getDeposit() != null && loan.getDeposit() > 0) {
				ubs.freezeMoney(loan.getUser().getId(), loan.getDeposit(),
						OperatorInfo.APPLY_LOAN,
						"发起借款，冻结保证金，借款ID:" + loan.getId());
			}
			if (log.isDebugEnabled())
	...
	@Override
	@Transactional(readOnly = false, rollbackFor = Exception.class)
	public void passApplyPublish(Loan loan) throws InsufficientBalance {
		// 审核通过
		loan.setStatus(LoanConstants.LoanStatus.RAISING);
		loan.setVerifyTime(new Date());
		ht.merge(loan);
    ...
    private void setPics(Loan loan) {
        ...
        if (lips != null && !(lips instanceof AbstractPersistentCollection)) {
            for (int i=0;i<lips.size();i++) {
                lips.get(i).setSeqNum(i+1);
                ht.saveOrUpdate(lips.get(i));
            }
        }
        if (gips != null && !(gips instanceof AbstractPersistentCollection)) {
            for (int j=0;j< gips.size();j++) {
                gips.get(j).setSeqNum(j+1);
                ht.saveOrUpdate(gips.get(j));
            }
        }
    }
	...
	@Override
	public boolean isOverPublishTime(String loanId) {
		Loan loan = ht.get(Loan.class, loanId);
		if (new Date().before(loan.getPublishTime())) {
			return false;
		}
		return true;
	}
	...
	@Override
	@Transactional(readOnly = false, rollbackFor = Exception.class)
	public void dealOverPublishTime(String loanId) {
		Loan loan = ht.get(Loan.class, loanId);
		ht.evict(loan);
		loan = ht.get(Loan.class, loanId, LockMode.UPGRADE);
		// 只有等待发布的借款，才能通过调度改成筹款中
		if (isOverPublishTime(loanId)
				&& LoanConstants.LoanStatus.WAITING_VERIFY.equals(loan
						.getStatus())) {
			loan.setStatus(LoanConstants.LoanStatus.RAISING);
			try {
				ht.merge(loan);
