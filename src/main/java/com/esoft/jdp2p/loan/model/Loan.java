...
public class Loan implements java.io.Serializable, Cloneable {
	...
	// 代偿者
	private User compensationUser;
	// 是否有抵押物
	private String hasPawn;

	// 是否有抵押物
	private String bannerPictureDes;
	...
	// private Double rate;
	/** 即投即生息，在项目开始计息之前的利率 */
	private Double rateIBOI;

	// 项目执行利率，仅限于竞标借款。注意，此处存储的不是百分比利率
	private Double actualRate;

	// XXX: guoyw,20150312,6.0修改字段
	/** 活动利率 */
	...
	@Lob
	@Basic(fetch = FetchType.LAZY)
	@Column(name = "banner_description", columnDefinition = "longtext")
	public String getBannerPictureDes() {
		return bannerPictureDes;
	}
	...
	@Column(name = "rate_iboi", precision = 22, scale = 0)
	public Double getRateIBOI() {
		if (rateIBOI == null) {
			return getRate();
		}
		return rateIBOI;
	...
	public void setRateIBOI(Double rateIBOI) {
		this.rateIBOI = rateIBOI;
	}
	...
	@Transient
	public Double getRateIBOIPercent() {
		if (this.getRateIBOI() != null) {
			return ArithUtil.round(this.getRateIBOI() * 100, 2);
		}
		return null;
	}
	...
	public Double getRate() {
		// XXX:guoyw,20150430,rate重新赋值:hdRate存在返回jkRate+hdRate,否则只返回jkRate
		if (this.getHdRate() != null) {
	...
	public void setInvestPassword(String investPassword) {
		// XXX:guoyw,20150430,若管理员未填写此项,默认"0"
		if (StringUtils.isBlank(investPassword)) {
			investPassword = LoanConstants.LoanInvestPassword.PSWD;
		}
	...
	@ManyToOne(fetch = FetchType.LAZY)
	@JoinColumn(name = "compensation_user_id")
	public User getCompensationUser() {
		return compensationUser;
	}

	public void setCompensationUser(User compensationUser) {
	...
	public void setBannerPictureDes(String bannerPictureDes) {
		this.bannerPictureDes = bannerPictureDes;
	}
	...
	public void setRateIBOIPercent(Double rateIBOIPercent) {
		if (rateIBOIPercent != null) {
			this.rateIBOI = ArithUtil.div(rateIBOIPercent, 100, 4);
		}
	}
