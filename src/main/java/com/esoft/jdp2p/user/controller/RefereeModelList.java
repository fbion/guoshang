package com.esoft.jdp2p.user.controller;
...
public class RefereeModelList extends EntityQuery<RefereeModel> {
	...
	private static final String lazyModel = "select "
			+ "new com.esoft.archer.user.model.RefereeModel(user.referrer,sum(invest.money),min(invest.time),max(invest.time),count(distinct user)) "
			+ "from Invest invest inner join invest.user user "
			+ "where user.referrer is not null and user.referrer != '' and "
			+ "invest.status in ('" + InvestStatus.BID_SUCCESS + "','"
			+ InvestStatus.REPAYING + "','" + InvestStatus.OVERDUE + "','" + InvestStatus.REPAYING_BACK + "','"
	...
	public String getRenderedHql() {
		String hql = super.getRenderedHql();
		if (! hql.contains("group by")) {
			hql = hql.concat(" group by user.referrer");
		}
		return hql;
	}
